...
const DoctorRegister = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      birthday: Yup.date()
        .max(new Date(Date.now() - 365 * 24 * 60 * 60 * 1000), "You must be at least 1 year old") // Ensure age is at least 1 year
    ...
    <main className="d-flex justify-content-center align-items-center py-5">
      <section className="body d-flex shadow ">
        <div className="photo w-50 rounded">
          <img src="src/assets/images/doctorReg.jpg" className="rounded border-0" alt="" />
        </div>
        ...
        <div className="form p-2 py-4 shadow">
          ...
          >
            ...
            {apiError && <div className="error">{apiError}</div>} {/* Show API error */}

            {["full_name", "email", "national_id", "phone_number", "password", "birthday", "address", "hospital", "clinic", "specialization"].map((field) => (
              <div className="input-group" key={field}>
                <label htmlFor={field}>{field.replace(/_/g, " ").replace(/\b\w/g, char => char.toUpperCase())}:</label>
                <input
